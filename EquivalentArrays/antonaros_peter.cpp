...
//implement the eqivalent function
bool equivalentShift(int x[], int y[], int length, int offset){
  for(int i=0; i< length; i++){
    if(x[i]!=y[(i+offset)%length]){
      return false;
    }
  }
  return true;
...
bool equivalent(int x[], int y[], int length){
  for(int offset=0; offset<length; offset++){
    if(equivalentShift(x,y,length,offset)){
      return true;
    }
  }
  return false; 
